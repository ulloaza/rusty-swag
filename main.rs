...
struct Args {
    ...
    /// Set the rate limit in requests per second (default: 30)
    #[arg(long, default_value = "30")]
    rate: u64,
    
    /// Enable exhaustive testing of parameter values
    #[arg(short = 'b', long)]
    brute: bool,
                        ...
                        if let Some(parameters) = &path_item_clone.parameters {
                            for param in parameters {
                                if param.location == "query" {
                                    let _example_value = if let Some(ex) = &param.example {
                                        format!("{}", ex)
                                    } else {
                                        "example".to_string()
                                    };
                                    // Note: Adding query params properly would require URL manipulation
